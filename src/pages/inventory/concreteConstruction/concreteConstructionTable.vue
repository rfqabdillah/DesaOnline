<template>
  <div class="col-sm-12">
    <add-edit-concreteConstruction-modal
      v-if="isModalVisible"
      :concreteConstruction-to-edit="concreteConstructionBeingEdited"
      @close="closeModal"
      ...
      <div class="table-responsive signal-table">
        <table class="table table-hover">
          <thead>
            <tr>
              <th scope="col">No</th>
              <th scope="col" @click="sortBy('namakonstruksibeton')">
                Nama Kontruksi Beton
                <i class="fa fa-sort-asc" v-if="sortColumn === 'namakonstruksibeton' && sortDirection === 'asc'"></i>
            ...
            <tr v-for="(item, index) in paginatedConcreteConstructions" :key="item.idkonstruksibeton">
              ...
              <td>
                <div class="btn-group">
                  ...
                  <button class="btn btn-danger sweet-11 btn-sm" type="button" @click="advancedDeleteAlert(item.idkonstruksibeton)">
...
<script>
import { getConcreteConstructions, deleteConcreteConstruction } from '@/services/referensi/concreteConstruction';
import AddEditConcreteConstructionModal from './addEditConcreteConstructionModal.vue';
import { useToast } from "vue-toastification";

export default {
  components: { AddEditConcreteConstructionModal },
  data() {
    return {
      ...
      perPage: 20,
      totalItems: 0,
      concreteConstructionBeingEdited: null,
      jumpPage: 1,
  ...
  methods: {
    ...
    openEditModal(item) {
      this.concreteConstructionBeingEdited = { ...item };
      this.isModalVisible = true;
    },
    closeModal() {
    ...
    async fetchConcreteConstructions() {
      try {
        const params = {
        ...
        const response = await getConcreteConstructions(params);
        
        const concreteConstructionData = response.data?.data || response.data?.[0]?.data || [];
        const meta = response.data?.[0]?.meta?.pagination || {};

        this.concreteConstructions = concreteConstructionData;
        this.totalItems = meta.total || concreteConstructionData.length;
        
      } catch (error) {
        console.error("Error fetching konstruksi beton:", error);
        this.$swal({
            icon: 'error',
            title: 'Oops...',
            text: 'Gagal memuat data konstruksi beton. Silakan coba lagi nanti.',
        });
      }
    },
      ...
      this.$swal({
        ...
        if (result.isConfirmed) {
          try {
            ...
            this.toast.success("Data konstruksi beton berhasil dihapus", { icon: 'fa fa-check' });
          } catch (error) {
            this.toast.error("Gagal menghapus data konstruksi beton", { icon: 'fa fa-times' });
          }
        }
      });
    },
