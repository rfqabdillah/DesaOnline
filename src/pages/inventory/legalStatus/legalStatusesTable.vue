<template>
  <div class="col-sm-12">
    <add-edit-legalStatus-modal
      v-if="isModalVisible"
      :legalStatus-to-edit="legalStatusBeingEdited"
      @close="closeModal"
    ...
    <div class="card">
      ...
      <div class="card-body">
        ...
        <div v-if="isFilterVisible" class="border p-3 mb-3 rounded filter-section">
          <div class="row g-2">
            <div class="col-md-4">
              <label for="filterName" class="form-label">Nama Status Hak</label>
              <input type="text" id="filterName" class="form-control" v-model="filters.namastatushak" placeholder="Filter berdasarkan nama status hak">
            </div>
          </div>
      ...
      <div class="table-responsive signal-table">
        <table class="table table-hover">
          <thead>
            <tr>
              <th scope="col">No</th>
              <th scope="col" @click="sortBy('namastatushak')">
                Nama Status Hak
                <i class="fa fa-sort-asc" v-if="sortColumn === 'namastatushak' && sortDirection === 'asc'"></i>
                <i class="fa fa-sort-desc" v-else-if="sortColumn === 'namastatushakhak' && sortDirection === 'desc'"></i>
          ...
          <tbody>
            <tr v-for="(item, index) in paginatedLegalStatuses" :key="item.idstatushak">
              <th scope="row">{{ (currentPage - 1) * perPage + index + 1 }}</th>
              <td>{{ item.namastatushak }}</td>
            ...
            <tr v-if="paginatedLegalStatuses.length === 0">
...
<script>
import { getLegalStatuses, deleteLegalStatus } from '@/services/referensi/legalStatus';
import AddEditLegalStatusModal from './addEditLegalStatusModal.vue';
import { useToast } from "vue-toastification";

export default {
  components: { AddEditLegalStatusModal },
  data() {
    return {
      legalStatuses: [],
      sortColumn: 'namastatushak',
      sortDirection: 'asc',
      isModalVisible: false,
      currentPage: 1,
      perPage: 20,
      totalItems: 0,
      legalStatusBeingEdited: null,
      jumpPage: 1,
  ...
  methods: {
    ...
    openAddModal() {
      this.legalStatusBeingEdited = null;
      this.isModalVisible = true;
    },
    openEditModal(item) {
      this.legalStatusBeingEdited = { ...item };
      this.isModalVisible = true;
    },
    closeModal() {
    ...
    async fetchLegalStatuses() {
      try {
        ...
        const response = await getLegalStatuses(params);
        
        const legalStatusData = response.data?.data || response.data?.[0]?.data || [];
        const meta = response.data?.[0]?.meta?.pagination || {};

        this.legalStatuses = legalStatusData;
        this.totalItems = meta.total || legalStatusData.length;
        
      } catch (error) {
        console.error("Error fetching status hak:", error);
        this.$swal({
            icon: 'error',
            title: 'Oops...',
            text: 'Gagal memuat data status hak. Silakan coba lagi nanti.',
        });
      }
    },
    advancedDeleteAlert(item) {
      this.$swal({
        title: `Hapus Status Hak "${item.namastatushak}"`,
        ...
        if (result.isConfirmed) {
          try {
            await deleteLegalStatus(item.idstatushak);
            if (this.legalStatuses.length === 1 && this.currentPage > 1) {
              this.currentPage--;
            } else {
              this.fetchLegalStatuses();
            }
            this.toast.success("Data status hak berhasil dihapus", { icon: 'fa fa-check' });
          } catch (error) {
            this.toast.error("Gagal menghapus data status hak", { icon: 'fa fa-times' });
          }
        }
      });
    },
    ...
    resetFilters() {
      this.filters.namastatushak = '';
      
      this.sortColumn = 'namastatushak';
      this.sortDirection = 'asc';
      this.currentPage = 1;
      this.fetchLegalStatuses();
    },
