<template>
  <div class="col-sm-12">
    <add-edit-acquisition-modal
      v-if="isModalVisible"
      :acquisition-to-edit="acquisitionBeingEdited"
      @close="closeModal"
    ...
    <div class="card">
      ...
      <div class="card-body">
        ...
        <div v-if="isFilterVisible" class="border p-3 mb-3 rounded filter-section">
          <div class="row g-2">
            <div class="col-md-4">
              <label for="filterName" class="form-label">Nama Perolehan</label>
              <input type="text" id="filterName" class="form-control" v-model="filters.namaperolehan" placeholder="Filter berdasarkan nam perolehan">
            </div>
          </div>
      ...
      <div class="table-responsive signal-table">
        <table class="table table-hover">
          <thead>
            <tr>
              <th scope="col">No</th>
              <th scope="col" @click="sortBy('namaperolehan')">
                Nama Perolehan
                <i class="fa fa-sort-asc" v-if="sortColumn === 'namaperolehan' && sortDirection === 'asc'"></i>
                <i class="fa fa-sort-desc" v-else-if="sortColumn === 'namaperolehan' && sortDirection === 'desc'"></i>
          ...
          <tbody>
            <tr v-for="(item, index) in paginatedAcquisitions" :key="item.idperolehan">
              <th scope="row">{{ (currentPage - 1) * perPage + index + 1 }}</th>
              <td>{{ item.namaperolehan }}</td>
              <td>
                <div class="btn-group">
                  <button class="btn btn-primary btn-sm" @click="openEditModal(item)">
                    <i class="fa fa-pencil"></i>
                  </button>
                  <button class="btn btn-danger sweet-11 btn-sm" type="button" @click="advancedDeleteAlert(item.idperolehan)">
                    <i class="fa fa-trash"></i>
                  </button>
                </div>
              </td>
            </tr>
            <tr v-if="paginatedAcquisitions.length === 0">
...
<script>
import { getAcquisitions, deleteAcquisition } from '@/services/referensi/acquisitions';
import AddEditAcquisitionModal from './addEditAcquisitionModal.vue';
import { useToast } from "vue-toastification";

export default {
  components: { AddEditAcquisitionModal },
  data() {
    return {
      acquisitions: [],
      sortColumn: '',
  ...
  methods: {
    openAddModal() {
      this.acquisitionBeingEdited = null;
      this.isModalVisible = true;
    },
    openEditModal(item) {
      this.acquisitionBeingEdited = { ...item };
      this.isModalVisible = true;
    },
    closeModal() {
    ...
    async fetchAcquisitions() {
      try {
        ...
        const response = await getAcquisitions(params);
        
        const acquisitionData = response.data?.data || response.data?.[0]?.data || [];
        const meta = response.data?.[0]?.meta?.pagination || {};

        this.acquisitions = acquisitionData;
        this.totalItems = meta.total || acquisitionData.length;
        
      } catch (error) {
        console.error("Error fetching perolehan:", error);
        this.$swal({
            icon: 'error',
            title: 'Oops...',
            text: 'Gagal memuat data perolehan. Silakan coba lagi nanti.',
        });
      }
    },
    advancedDeleteAlert(id) {
      this.$swal({
        title: 'Hapus Data Perolehan',
        ...
        if (result.isConfirmed) {
          try {
            ...
            if (this.acquisitions.length === 1 && this.currentPage > 1) {
              this.currentPage--;
            } else {
              this.fetchAcquisitions();
            }
            this.toast.success("Data perolehan berhasil dihapus", { icon: 'fa fa-check' });
          } catch (error) {
            this.toast.error("Gagal menghapus data perolehan", { icon: 'fa fa-times' });
          }
        }
      });
    },
