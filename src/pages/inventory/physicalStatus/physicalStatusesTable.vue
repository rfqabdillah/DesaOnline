<template>
  <div class="col-sm-12">
    <add-edit-physicalStatus-modal
      v-if="isModalVisible"
      :physicalStatus-to-edit="physicalStatusBeingEdited"
      @close="closeModal"
    ...
    <div class="card">
      <div class="card-header">
        <h3>Daftar Nama Keberadaan Fisik</h3>
      </div>
      ...
      <div class="card-body">
        ...
        <div v-if="isFilterVisible" class="border p-3 mb-3 rounded filter-section">
          <div class="row g-2">
            <div class="col-md-4">
              <label for="filterName" class="form-label">Nama Keberadaan Fisik</label>
              <input type="text" id="filterName" class="form-control" v-model="filters.namakeberadaanfisik" placeholder="Filter berdasarkan nama keberadaan fisik">
            </div>
          </div>
      ...
      <div class="table-responsive signal-table">
        <table class="table table-hover">
          <thead>
            <tr>
              <th scope="col">No</th>
              <th scope="col" @click="sortBy('namakeberadaanfisik')">
                Nama Keberadaan Fisik
                <i class="fa fa-sort-asc" v-if="sortColumn === 'namakeberadaanfisik' && sortDirection === 'asc'"></i>
                <i class="fa fa-sort-desc" v-else-if="sortColumn === 'namakeberadaanfisik' && sortDirection === 'desc'"></i>
          ...
          <tbody>
            <tr v-for="(item, index) in paginatedPhysicalStatuses" :key="item.idkeberadaanfisik">
              <th scope="row">{{ (currentPage - 1) * perPage + index + 1 }}</th>
              <td>{{ item.namakeberadaanfisik }}</td>
            ...
            <tr v-if="paginatedPhysicalStatuses.length === 0">
...
<script>
import { getPhysicalStatuses, deletePhysicalStatus } from '@/services/referensi/physicalStatus';
import AddEditPhysicalStatusModal from './addEditPhysicalStatusModal.vue';
import { useToast } from "vue-toastification";

export default {
  components: { AddEditPhysicalStatusModal },
  data() {
    return {
      physicalStatuses: [],
      sortColumn: 'namakeberadaanfisik',
      sortDirection: 'asc',
      isModalVisible: false,
      currentPage: 1,
      perPage: 20,
      totalItems: 0,
      physicalStatusBeingEdited: null,
      jumpPage: 1,
  ...
  computed: {
    ...
    paginatedPhysicalStatuses() {
      return this.physicalStatuses;
    },
    totalPages() {
  ...
  async mounted() {
    this.toast = useToast();
    this.loadUserRole();
    await this.fetchPhysicalStatuses();
  },
  methods: {
    loadUserRole() {
    ...
    openAddModal() {
      this.physicalStatusBeingEdited = null;
      this.isModalVisible = true;
    },
    openEditModal(item) {
      this.physicalStatusBeingEdited = { ...item };
      this.isModalVisible = true;
    },
    closeModal() {
    ...
    async fetchPhysicalStatuses() {
      try {
        ...
        const response = await getPhysicalStatuses(params);
        
        const physicalStatusData = response.data?.data || response.data?.[0]?.data || [];
        const meta = response.data?.[0]?.meta?.pagination || {};

        this.physicalStatuses = physicalStatusData;
        this.totalItems = meta.total || physicalStatusData.length;
        
      } catch (error) {
        console.error("Error fetching keberadaan fisik:", error);
        this.$swal({
            icon: 'error',
            title: 'Oops...',
            text: 'Gagal memuat data keberadaan fisik. Silakan coba lagi nanti.',
        });
      }
    },
    advancedDeleteAlert(item) {
      this.$swal({
        title: `Hapus Keberadaan Fisik "${item.namakeberadaanfisik}"`,
        ...
        if (result.isConfirmed) {
          try {
            await deletePhysicalStatus(item.idkeberadaanfisik);
            if (this.physicalStatuses.length === 1 && this.currentPage > 1) {
              this.currentPage--;
            } else {
              this.fetchPhysicalStatuses();
            }
            this.toast.success("Data keberadaan fisik berhasil dihapus", { icon: 'fa fa-check' });
          } catch (error) {
            this.toast.error("Gagal menghapus data keberadaan fisik", { icon: 'fa fa-times' });
          }
        }
      });
    },
    ...
    resetFilters() {
      this.filters.namakeberadaanfisik = '';
      
      this.sortColumn = 'namakeberadaanfisik';
      this.sortDirection = 'asc';
      this.currentPage = 1;
      this.fetchPhysicalStatuses();
    },
