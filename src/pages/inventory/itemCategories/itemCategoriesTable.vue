<template>
  <div class="col-sm-12">
    <add-edit-itemCategory-modal
      v-if="isModalVisible"
      :itemCategory-to-edit="itemCategoryBeingEdited"
      @close="closeModal"
    ...
    <div class="card">
      ...
      <div class="card-body">
        ...
        <div v-if="isFilterVisible" class="border p-3 mb-3 rounded filter-section">
          <div class="row g-2">
            <div class="col-md-4">
              <label for="filterName" class="form-label">Kategori Barang</label>
              <input type="text" id="filterName" class="form-control" v-model="filters.namakategoribarang" placeholder="Filter berdasarkan nama kategori barang">
            </div>
          </div>
      ...
      <div class="table-responsive signal-table">
        <table class="table table-hover">
          <thead>
            <tr>
              <th scope="col">No</th>
              <th scope="col" @click="sortBy('namakategoribarang')">
                Kategori Barang
                <i class="fa fa-sort-asc" v-if="sortColumn === 'namakategoribarang' && sortDirection === 'asc'"></i>
                <i class="fa fa-sort-desc" v-else-if="sortColumn === 'namakategoribarang' && sortDirection === 'desc'"></i>
          ...
          <tbody>
            <tr v-for="(item, index) in paginatedItemCategories" :key="item.idkategoribarang">
              <th scope="row">{{ (currentPage - 1) * perPage + index + 1 }}</th>
              <td>{{ item.namakategoribarang }}</td>
              <td v-if="isSuperAdmin">
                <div class="btn-group">
                  <button class="btn btn-primary btn-sm" @click="openEditModal(item)">
                    <i class="fa fa-pencil"></i>
                  </button>
                  <button class="btn btn-danger sweet-11 btn-sm" type="button" @click="advancedDeleteAlert(item.idkategoribarang)">
                    <i class="fa fa-trash"></i>
                  </button>
                </div>
              </td>
            </tr>
            <tr v-if="paginatedItemCategories.length === 0">
...
<script>
import { getItemCategories, deleteItemCategory } from '@/services/referensi/itemCategories';
import AddEditItemCategoryModal from './addEditItemCategoryModal.vue';
import { useToast } from "vue-toastification";

export default {
  components: { AddEditItemCategoryModal },
  data() {
    return {
      itemCategories: [],
      sortColumn: 'namakategoribarang',
      sortDirection: 'asc',
      isModalVisible: false,
      currentPage: 1,
      perPage: 20,
      totalItems: 0,
      itemCategoryBeingEdited: null,
      jumpPage: 1,
  ...
  methods: {
    ...
    openAddModal() {
      this.itemCategoryBeingEdited = null;
      this.isModalVisible = true;
    },
    openEditModal(item) {
      this.itemCategoryBeingEdited = { ...item };
      this.isModalVisible = true;
    },
    closeModal() {
    ...
    async fetchItemCategories() {
      try {
        ...
        const itemCategoryData = response.data?.data || response.data?.[0]?.data || [];
        const meta = response.data?.[0]?.meta?.pagination || {};

        this.itemCategories = itemCategoryData;
        this.totalItems = meta.total || itemCategoryData.length;
        ...
        this.$swal({
            icon: 'error',
            title: 'Oops...',
            text: 'Gagal memuat data kategori barang. Silakan coba lagi nanti.',
        });
      }
    },
    advancedDeleteAlert(item) {
      this.$swal({
        title: `Hapus Kategori "${item.namakategoribarang}"`,
        ...
        if (result.isConfirmed) {
          try {
            await deleteItemCategory(item.idkategoribarang);
            if (this.itemCategories.length === 1 && this.currentPage > 1) {
              this.currentPage--;
            } else {
              this.fetchItemCategories();
            }
            this.toast.success("Data kategori barang berhasil dihapus", { icon: 'fa fa-check' });
          } catch (error) {
            this.toast.error("Gagal menghapus data kategori barang", { icon: 'fa fa-times' });
          }
        }
      });
    },
    ...
    resetFilters() {
      this.filters.namakategoribarang = '';
      
      this.sortColumn = 'namakategoribarang';
      this.sortDirection = 'asc';
      this.currentPage = 1;
      this.fetchItemCategories();
    },
