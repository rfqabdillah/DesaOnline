        ...
        <div v-if="isFilterVisible" class="border p-3 mb-3 rounded filter-section">
          <div class="row g-2">
            <div class="col-md-4">
              ...
              <input type="text" id="filterName" class="form-control" v-model="filters.namaperiodelaporan" placeholder="Filter berdasarkan nama periode">
            </div>
          </div>
...
export default {
  ...
  methods: {
    ...
    advancedDeleteAlert(item) {
      this.$swal({
        title: `Hapus "${item.namaperiodelaporan}"`,
        ...
        if (result.isConfirmed) {
          try {
            await deleteReportPeriod(item.idperiodelaporan);
