<template>
  <div class="col-sm-12">
    <add-edit-businessField-modal
      v-if="isModalVisible"
      :businessField-to-edit="businessFieldBeingEdited"
      @close="closeModal"
    ...
    <div class="card">
      <div class="card-header">
        <h3>Daftar Nama Bidang Usaha</h3>
      </div>
      ...
      <div class="card-body">
        ...
        <div v-if="isFilterVisible" class="border p-3 mb-3 rounded filter-section">
          <div class="row g-2">
            <div class="col-md-4">
              <label for="filterName" class="form-label">Nama Bidang Usaha</label>
              <input type="text" id="filterName" class="form-control" v-model="filters.namabidangusaha" placeholder="Filter berdasarkan nama bidang usaha">
            </div>
          </div>
      ...
      <div class="table-responsive signal-table">
        <table class="table table-hover">
          ...
          <tbody>
            <tr v-for="(item, index) in paginatedBusinessFields" :key="item.idbidangusaha">
              <th scope="row">{{ (currentPage - 1) * perPage + index + 1 }}</th>
              <td>{{ item.namabidangusaha }}</td>
              <td>
                <div class="btn-group">
                  <button class="btn btn-primary btn-sm" @click="openEditModal(item)">
                    <i class="fa fa-pencil"></i>
                  </button>
                  <button class="btn btn-danger sweet-11 btn-sm" type="button" @click="advancedDeleteAlert(item.idbidangusaha)">
                    <i class="fa fa-trash"></i>
                  </button>
                </div>
              </td>
            </tr>
            <tr v-if="paginatedBusinessFields.length === 0">
...
<script>
import { getBusinessFields, deleteBusinessField } from '@/services/referensi/businessFields';
import AddEditBusinessFieldModal from './addEditBusinessFieldModal.vue';
import { useToast } from "vue-toastification";

export default {
  components: { AddEditBusinessFieldModal },
  data() {
    return {
      businessFields: [],
      sortColumn: '',
  ...
  methods: {
    openAddModal() {
      this.businessFieldBeingEdited = null;
      this.isModalVisible = true;
    },
    openEditModal(item) {
      this.businessFieldBeingEdited = { ...item };
      this.isModalVisible = true;
    },
    closeModal() {
    ...
    async fetchBusinessFields() {
      try {
        ...
        const response = await getBusinessFields(params);
        
        const businessFieldData = response.data?.data || response.data?.[0]?.data || [];
        const meta = response.data?.[0]?.meta?.pagination || {};

        this.businessFields = businessFieldData;
        this.totalItems = meta.total || businessFieldData.length;
        
      } catch (error) {
        console.error("Error fetching bidang usaha:", error);
        this.$swal({
            icon: 'error',
            title: 'Oops...',
            text: 'Gagal memuat data bidang usaha. Silakan coba lagi nanti.',
        });
      }
    },
    advancedDeleteAlert(id) {
      this.$swal({
        title: 'Hapus Data Periode Bidang Usaha',
        ...
        if (result.isConfirmed) {
          try {
            await deleteBusinessField(id);
            if (this.businessFields.length === 1 && this.currentPage > 1) {
              this.currentPage--;
            } else {
              this.fetchBusinessFields();
            }
            this.toast.success("Data bidang usaha berhasil dihapus", { icon: 'fa fa-check' });
          } catch (error) {
            this.toast.error("Gagal menghapus data bidang usaha", { icon: 'fa fa-times' });
          }
        }
      });
    },
    ...
    resetFilters() {
      this.filters.namabidangbidangusaha = '';
