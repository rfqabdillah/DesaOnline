...
export default {
  ...
  methods: {
    ...
    advancedDeleteAlert(item) {
      this.$swal({
        title: `Hapus Bidang Usaha "${item.namabidangusaha}"`,
        ...
        if (result.isConfirmed) {
          try {
            await deleteBusinessField(item.idbidangusaha);
    ...
    resetFilters() {
      this.filters.namabidangusaha = '';
      
      this.sortColumn = 'namabidangusaha';
      this.sortDirection = 'asc';
      this.currentPage = 1;
      this.fetchBusinessFields();
    },
