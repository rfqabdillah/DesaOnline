            ...
            <template v-else>
              <tr v-for="(item, index) in documents" :key="item.iddokumen">
                ...
                <td v-if="canPerformActions">
                  <div class="btn-group">
                    <button class="btn btn-success btn-sm" @click="openFile(item.dokumen)" title="Lihat Dokumen">
...
export default {
  ...
  methods: {
    ...
    advancedDeleteAlert(item) { 
      this.$swal({
        title: `Hapus Dokumen "${item.namadokumen}"`,
        ...
        if (result.isConfirmed) {
          try {
            await deleteDocument(item.iddokumen); 
