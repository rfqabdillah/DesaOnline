<template>
  <div class="col-sm-12">
    ...
    <add-edit-articleCategory-modal
      v-if="isModalVisible"
      :articleCategory-to-edit="articleCategoryBeingEdited"
      @close="closeModal"
    ...
    <div class="card">
      ...
      <div class="card-body">
        ...
        <div v-if="isFilterVisible" class="border p-3 mb-3 rounded filter-section">
            <div class="row g-3">
              <div class="col-md-3">
              <label for="filterArticleCategoryName" class="form-label">Nama Kategori Artikel</label>
              <input type="text" id="filterArticleCategoryName" class="form-control" v-model="filters.namakategoriartikel" placeholder="Filter berdasarkan nama kategori">
              </div>
              <div class="col-md-3">
      ...
      <div class="table-responsive signal-table">
        <table class="table table-hover">
          ...
          <tbody>
            ...
            <template v-else>
              <tr v-for="(item, index) in articleCategories" :key="item.idkategoriartikel">
                <th scope="row"> {{ (currentPage - 1) * perPage + index + 1 }}</th>
                <td>{{ item.namakategoriartikel|| '-' }}</td>
                ...
                <td>
                  <div class="btn-group">
                    ...
                    <button class="btn btn-danger sweet-11 btn-sm" type="button" @click="advancedDeleteAlert(item.idkategoriartikel)" title="Hapus Data">
                      <i class="fa fa-trash"></i>
                    </button>
                  </div>
                </td>
              </tr>
              <tr v-if="paginatedArticleCategories.length === 0">
...
<script>
import { getArticleCategories, deleteArticleCategory } from '@/services/general/website/articleCategory'; 
import AddEditArticleCategoryModal from './addEditArticlesCategoryModal.vue';
import { useToast } from "vue-toastification";

export default {
  components: { AddEditArticleCategoryModal }, 
  data() {
    return {
      articleCategories: [],
      isLoading: false, 
      ...
      perPage: 10,
      totalItems: 0,
      articleCategoryBeingEdited: null,
  ...
  async mounted() {
    this.toast = useToast();
    await this.fetchArticleCategories();
  },
  methods: {
    openAddModal() {
      this.articleCategoryBeingEdited = null;
      this.isModalVisible = true;
    },
    openEditModal(item) {
      this.articleCategoryBeingEdited = { ...item };
      this.isModalVisible = true;
    },
    closeModal() {
    ...
    async fetchArticleCategories() {
      ...
      try {
        ...
        const response = await getArticleCategories(params);
        const articleCategoryData = response.data?.data || response.data?.[0]?.data || [];
        const meta = response.data?.meta?.pagination || response.data?.[0]?.meta?.pagination || {};

        this.articleCategories = articleCategoryData;
        this.totalItems = meta.total || articleCategoryData.length;
        
      } catch (error) {
        console.error("Error fetching articleCategories:", error);
        this.toast.error("Gagal memuat data kategori artikel", { icon: 'fa fa-times' });
      } finally {
    ...
    advancedDeleteAlert(id) { 
      this.$swal({
        title: 'Hapus Data Kategori Artikel',
        ...
        if (result.isConfirmed) {
          try {
            ...
            if (this.articleCategories.length === 1 && this.currentPage > 1) {
              this.currentPage--;
            } else {
              this.fetchArticleCategories();
            }
            this.toast.success("Data kategori artikel berhasil dihapus", { icon: 'fa fa-check' });
          } catch (error) {
            this.toast.error("Gagal menghapus data kategori artikel", { icon: 'fa fa-times' });
          }
        }
      });
    },
