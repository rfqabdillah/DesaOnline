    ...
    <div class="card">
      ...
      <div class="card-body">
        ...
        <div v-if="isFilterVisible" class="border p-3 mb-3 rounded filter-section">
            <div class="row g-3">
              <div class="col-md-3">
                <label for="filterUsahaName" class="form-label">Nama Usaha</label>
                <input type="text" id="filterUsahaName" class="form-control" v-model="filters.namausaha" placeholder="filter berdasarkan nama dokumen">
              </div>
              <div class="col-md-3">
                <label for="filterPeriodName" class="form-label">Nama Periode laporan</label>
                <input type="text" id="filterPeriodName" class="form-control" v-model="filters.namaperiodelaporan" placeholder="Filter berdasarkan nama periode laporan">
              </div>
              <div class="col-md-3">
        ...
        <table class="table table-hover">
          <thead>
            <tr>
              ...
              <th scope="col" @click="sortBy('tanggallaporan')">
                Tanggal Laporan
                <i class="fa fa-sort-asc" v-if="sortColumn === 'tanggallaporan' && sortDirection === 'asc'"></i>
                <i class="fa fa-sort-desc" v-else-if="sortColumn === 'tanggallaporan' && sortDirection === 'desc'"></i>
                <i class="fa fa-sort" v-else></i>
              </th>
          ...
          <tbody>
            ...
            <template v-else>
              <tr v-for="(item, index) in reports" :key="item.iddokumen">
                <th scope="row"> {{ (currentPage - 1) * perPage + index + 1 }}</th>
                <td>
                  <img :src="item.usaha?.logousaha" alt="Logo" style="height: 50px; width: auto;" v-if="item.usaha?.logousaha" />
...
<script>
import { getReports, deleteReport } from '@/services/general/enterprise/reports';
import AddEditReportModal from './addEditReportModal.vue';
import { useToast } from "vue-toastification";

export default {
  components: { AddEditReportModal }, 
  data() {
    return {
      reports: [],
      isLoading: false, 
  ...
  methods: {
    ...
    openFile(fileUrl) {
      if (fileUrl) {
        ...
        this.toast.info("Tidak ada file laporan usaha yang tersedia", { icon: 'fa fa-info-circle' });
      }
    },
    ...
    async fetchReports() {
      ...
      try {
        ...
        const reportData = response.data?.data || response.data?.[0]?.data || [];
        const meta = response.data?.meta?.pagination || response.data?.[0]?.meta?.pagination || {};

        this.reports = reportData;
        this.totalItems = meta.total || reportData.length;
        
      } catch (error) {
        console.error("Error fetching reports:", error);
        this.toast.error("Gagal memuat data laporan usaha", { icon: 'fa fa-times' });
      } finally {
      ...
      this.$swal({
        ...
        if (result.isConfirmed) {
          try {
            ...
            this.toast.success("Data laporan usaha berhasil dihapus", { icon: 'fa fa-check' });
          } catch (error) {
            this.toast.error("Gagal menghapus data laporan usaha", { icon: 'fa fa-times' });
          }
        }
      });
    },
