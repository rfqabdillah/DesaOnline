  ...
  <div class="modal-overlay" @click="handleOverlayClick">
    <div class="modal-content" @click.stop>
      <div class="modal-header">
        <h5 class="modal-title">{{ isEditMode ? 'Edit Data Status Perangkat' : 'Tambah Data Status Perangkat' }}</h5>
      ...
      <div class="modal-body">
        <form @submit.prevent="submitForm">
          <div class="mb-3">
            <label for="namastatus" class="form-label">Nama Status Perangkat</label>
            <input 
              type="text" 
              class="form-control" 
              id="namaStatus" 
              placeholder="Masukkan nama status perangkat"
              v-model="formData.namastatus" required>
          </div>
...
<script>
import { addOfficialStatus, updateOfficialStatus } from '@/services/referensi/officialStatuses';
import { useToast } from "vue-toastification";

export default {
  name: 'editOfficialStatusModal',
  props: {
  ...
  computed: {
    isEditMode() {
      return !!this.officialStatusToEdit;
    }
  },
  watch: {
    officialStatusToEdit: {
  ...
  methods: {
    ...
    async submitForm() {
      ...
      try {
        ...
        this.toast.error("Gagal menyimpan data status perangkat", { icon: 'fa fa-times' });
      } finally {
