<template>
  <div class="col-sm-12">
    <add-edit-officialStatus-modal
      v-if="isModalVisible"
      :officialStatus-to-edit="officialStatusBeingEdited"
      @close="closeModal"
    ...
    <div class="card">
      <div class="card-header">
        <h3>Daftar Nama Status Perangkat</h3>
      </div>
      ...
      <div class="card-body">
        ...
        <div v-if="isFilterVisible" class="border p-3 mb-3 rounded filter-section">
          <div class="row g-2">
            <div class="col-md-4">
              <label for="filterName" class="form-label">Nama Status Perangkat</label>
              <input type="text" id="filterName" class="form-control" v-model="filters.namastatus" placeholder="Filter berdasarkan nama status perangkat">
            </div>
          </div>
      ...
      <div class="table-responsive signal-table">
        <table class="table table-hover">
          ...
          <tbody>
            <tr v-for="(item, index) in paginatedOfficialStatuses" :key="item.idstatus">
              ...
              <td>
                <div class="btn-group">
                  ...
                  <button class="btn btn-danger sweet-11 btn-sm" type="button" @click="advancedDeleteAlert(item.idstatus)">
                    <i class="fa fa-trash"></i>
                  </button>
                </div>
              </td>
            </tr>
            <tr v-if="paginatedOfficialStatuses.length === 0">
...
<script>
import { getOfficialStatuses, deleteOfficialStatus } from '@/services/referensi/officialStatuses';
import AddEditOfficialStatusModal from './addEditOfficialStatusModal.vue';
import { useToast } from "vue-toastification";

export default {
  components: { AddEditOfficialStatusModal },
  data() {
    return {
      officialStatuses: [],
      sortColumn: '',
      ...
      perPage: 20,
      totalItems: 0,
      officialStatusBeingEdited: null,
      jumpPage: 1,
  ...
  computed: {
    paginatedOfficialStatuses() {
      return this.officialStatuses;
    },
    totalPages() {
  ...
  async mounted() {
    this.toast = useToast();
    await this.fetchOfficialStatuses();
  },
  methods: {
    openAddModal() {
      this.officialStatusBeingEdited = null;
      this.isModalVisible = true;
    },
    openEditModal(item) {
      this.officialStatusBeingEdited = { ...item };
      this.isModalVisible = true;
    },
    closeModal() {
    ...
    async fetchOfficialStatuses() {
      try {
        ...
        const response = await getOfficialStatuses(params);
        
        const officialStatusData = response.data?.data || response.data?.[0]?.data || [];
        const meta = response.data?.[0]?.meta?.pagination || {};

        this.officialStatuses = officialStatusData;
        this.totalItems = meta.total || officialStatusData.length;
        
      } catch (error) {
        console.error("Error fetching status perangkat:", error);
        this.$swal({
            icon: 'error',
            title: 'Oops...',
            text: 'Gagal memuat data status perangkat. Silakan coba lagi nanti.',
        });
      }
    },
    advancedDeleteAlert(id) {
      this.$swal({
        title: 'Hapus Data Status Perangkat',
        ...
        if (result.isConfirmed) {
          try {
            await deleteOfficialStatus(id);
            if (this.officialStatuses.length === 1 && this.currentPage > 1) {
              this.currentPage--;
            } else {
              this.fetchOfficialStatuses();
            }
            this.toast.success("Data status perangkat berhasil dihapus", { icon: 'fa fa-check' });
          } catch (error) {
            this.toast.error("Gagal menghapus data status perangkat", { icon: 'fa fa-times' });
          }
        }
      });
    },
