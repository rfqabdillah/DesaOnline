      ...
      <div class="card-body">
        ...
        <div v-if="isFilterVisible" class="border p-3 mb-3 rounded filter-section">
          <div class="row g-3">
            ...
            <div class="col-md-3">
              <label for="filterStartYear" class="form-label">Tahun Awal</label>
              <input type="text" id="filterStartYear" class="form-control" v-model="filters.tahunawal" placeholder="Filter berdasarkan tahun awal">
            </div>
            <div class="col-md-3">
              <label for="filterEndYear" class="form-label">Tahun Akhir</label>
              <input type="text" id="filterEndYear" class="form-control" v-model="filters.tahunakhir" placeholder="Filter berdasarkan tahun akhir">
            </div>
            <div class="col-md-3">
        ...
        <table class="table table-hover">
          <thead>
            <tr>
              ...
              <th scope="col" @click="sortBy('tahunawal')">
                Tahun Awal
                <i class="fa fa-sort-asc" v-if="sortColumn === 'tahunawal' && sortDirection === 'asc'"></i>
                <i class="fa fa-sort-desc" v-else-if="sortColumn === 'tahunawal' && sortDirection === 'desc'"></i>
                <i class="fa fa-sort" v-else></i>
              </th>
              <th scope="col" @click="sortBy('tahunakhir')">
                Tahun Akhir
                <i class="fa fa-sort-asc" v-if="sortColumn === 'tahunakhir' && sortDirection === 'asc'"></i>
                <i class="fa fa-sort-desc" v-else-if="sortColumn === 'tahunakhir' && sortDirection === 'desc'"></i>
                <i class="fa fa-sort" v-else></i>
              </th>
          ...
          <tbody>
            ...
            <template v-else>
              <tr v-for="(item, index) in paginatedPeriods" :key="item.idperiode">
...
<script>
import { getPeriods, deletePeriod } from '@/services/general/villageGovernment/tenurePeriods'; 
import AddEditPeriodModal from './addEditPeriodModal.vue';
import { useToast } from "vue-toastification";

export default {
  components: { AddEditPeriodModal }, 
  data() {
    return {
      periods: [],
      isLoading: false, 
  ...
  methods: {
    ...
    async fetchPeriods() {
      ...
      try {
        ...
        const periodData = response.data?.data || response.data?.[0]?.data || [];
        const meta = response.data?.meta?.pagination || response.data?.[0]?.meta?.pagination || {};

        this.periods = periodData;
        this.totalItems = meta.total || periodData.length;
        
      } catch (error) {
        console.error("Error fetching periods:", error);
        this.toast.error("Gagal memuat data periode jabatan", { icon: 'fa fa-times' });
      } finally {
      ...
      this.$swal({
        ...
        if (result.isConfirmed) {
          try {
            ...
            this.toast.success("Data periode jabatan berhasil dihapus", { icon: 'fa fa-check' });
          } catch (error) {
            this.toast.error("Gagal menghapus data periode jabatan", { icon: 'fa fa-times' });
          }
        }
      });
    },
