    ...
    <div class="modal-overlay" @click="handleOverlayClick">
      <div class="modal-content" @click.stop>
        <div class="modal-header">
          <h5 class="modal-title">{{ isEditMode ? 'Edit Data Periode Jabatan' : 'Tambah Data Periode Jabatan' }}</h5>
          ...
          <form @submit.prevent="submitForm">
            ...
            <div class="row">
              <div class="col-md-6 mb-3">
                ...
                <input type="number" class="form-control" v-model="formData.tahunawal" placeholder="YYYY" required />
              </div>
              <div class="col-md-6 mb-3">
                <label class="form-label">Tahun Akhir</label>
                <input type="number" class="form-control" v-model="formData.tahunakhir" placeholder="YYYY" required />
              </div>
            </div>
...
<script>
import { addPeriod, updatePeriod } from '@/services/general/villageGovernment/tenurePeriods'; 
...
export default {
  name: 'addEditPeriodModal',
  props: {
    periodToEdit: { type: Object, default: null }
  },
  data() {
    return {
    ...
    periodToEdit: {
      handler(newData) {
        if (newData) {
          ...
          this.formData.tahunawal = newData.tahunawal;
          this.formData.tahunakhir = newData.tahunakhir;
  ...
  methods: {
    ...
    async submitForm() {
      ...
      data.append('record[tahunawal]', this.formData.tahunawal);
      data.append('record[tahunakhir]', this.formData.tahunakhir);
      ...
      try {
        if (this.isEditMode) {
          ...
          await updatePeriod(this.periodToEdit.idperiode, data);
          this.toast.success("Data periode jabatan berhasil diperbarui");
        } else {
          await addPeriod(data);
          this.toast.success("Data periode jabatan berhasil ditambah");
        }
        ...
        this.toast.error("Gagal menyimpan data periode jabatan");
      } finally {
