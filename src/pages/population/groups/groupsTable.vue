    ...
    <div class="card">
      ...
      <div class="card-body">
        ...
        <div v-if="isFilterVisible" class="border p-3 mb-3 rounded filter-section">
          <div class="row g-3">
            <div class="col-md-3">
              <label for="filterGroupName" class="form-label">Nama Kelompok</label>
              <input type="text" id="filterGroupName" class="form-control" v-model="filters.namkelompok" placeholder="Filter berdasarkan nama kelompok...">
            </div>
            <div class="col-md-3">
        ...
        <table class="table table-hover">
          <thead>
            <tr>
              ...
              <th scope="col" @click="sortBy('namakelompok')">
                Nama Kelompok
                <i class="fa fa-sort-asc" v-if="sortColumn === 'namakelompok' && sortDirection === 'asc'"></i>
                <i class="fa fa-sort-desc" v-else-if="sortColumn === 'namakelompok' && sortDirection === 'desc'"></i>
                <i class="fa fa-sort" v-else></i>
              </th>
              <th scope="col" @click="sortBy('namakategorikelompok')">
                Nama Kategori Kelompok
          ...
          <tbody>
            ...
            <template v-else>
              <tr v-for="(item, index) in paginatedGroups" :key="item.idkelompok">
                <th scope="row">{{ (currentPage - 1) * perPage + index + 1 }}</th>
                <td>
                  <img :src="item.logo" alt="Logo" style="height: 50px; width: auto;" v-if="item.logo" />
...
<script>
import { getGroups, deleteGroup } from '@/services/general/population/groups'; 
import AddEditGroupModal from './addEditGroupModal.vue';
import { useToast } from "vue-toastification";

export default {
  components: { AddEditGroupModal }, 
  data() {
    return {
      groups: [],
      isLoading: false, 
  ...
  methods: {
    ...
    async fetchGroups() {
      ...
      try {
        ...
        const groupData = response.data?.data || response.data?.[0]?.data || [];
        const meta = response.data?.meta?.pagination || response.data?.[0]?.meta?.pagination || {};

        this.groups = groupData;
        this.totalItems = meta.total || groupData.length;
        
      } catch (error) {
        console.error("Error fetching groups:", error);
        this.toast.error("Gagal memuat data kelompok", { icon: 'fa fa-times' });
      } finally {
      ...
      this.$swal({
        ...
        if (result.isConfirmed) {
          try {
            ...
            this.toast.success("Data kelompok berhasil dihapus", { icon: 'fa fa-check' });
          } catch (error) {
            this.toast.error("Gagal menghapus data kelompok", { icon: 'fa fa-times' });
          }
        }
      });
    },
