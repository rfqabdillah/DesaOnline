<template>
  <div class="col-sm-12">
    <add-edit-regulationType-modal
      v-if="isModalVisible"
      :regulationType-to-edit="regulationTypeBeingEdited"
      @close="closeModal"
    ...
    <div class="card">
      <div class="card-header">
        <h3>Daftar Nama Jenis Regulasi</h3>
      </div>
      ...
      <div class="card-body">
        ...
        <div v-if="isFilterVisible" class="border p-3 mb-3 rounded filter-section">
          <div class="row g-2">
            <div class="col-md-4">
              <label for="filterName" class="form-label">Nama Jenis Regulasi</label>
              <input type="text" id="filterName" class="form-control" v-model="filters.namajenisregulasi" placeholder="Filter berdasarkan nama jenis regulasi">
            </div>
          </div>
      ...
      <div class="table-responsive signal-table">
        <table class="table table-hover">
          <thead>
            <tr>
              <th scope="col">No</th>
              <th scope="col" @click="sortBy('namajenisregulasi')">
                Nama Jenis Regulasi
                <i class="fa fa-sort-asc" v-if="sortColumn === 'namajenisregulasi' && sortDirection === 'asc'"></i>
                <i class="fa fa-sort-desc" v-else-if="sortColumn === 'namajenisregulasi' && sortDirection === 'desc'"></i>
          ...
          <tbody>
            <tr v-for="(item, index) in paginatedRegulationTypes" :key="item.idjenisregulasi">
              <th scope="row">{{ (currentPage - 1) * perPage + index + 1 }}</th>
              <td>{{ item.namajenisregulasi }}</td>
            ...
            <tr v-if="paginatedRegulationTypes.length === 0">
...
<script>
import { getRegulationTypes, deleteRegulationType } from '@/services/referensi/regulationTypes';
import AddEditRegulationTypeModal from './addEditRegulationTypeModal.vue';
import { useToast } from "vue-toastification";

export default {
  components: { AddEditRegulationTypeModal },
  data() {
    return {
      regulationTypes: [],
      sortColumn: 'namajenisregulasi',
      sortDirection: 'asc',
      isModalVisible: false,
      currentPage: 1,
      perPage: 20,
      totalItems: 0,
      regulationTypeBeingEdited: null,
      jumpPage: 1,
  ...
  async mounted() {
    this.toast = useToast();
    this.loadUserRole();
    await this.fetchRegulationTypes();
  },
  methods: {
    loadUserRole() {
    ...
    openAddModal() {
      this.regulationTypeBeingEdited = null;
      this.isModalVisible = true;
    },
    openEditModal(item) {
      this.regulationTypeBeingEdited = { ...item };
      this.isModalVisible = true;
    },
    closeModal() {
    ...
    async fetchRegulationTypes() {
      try {
        ...
        const response = await getRegulationTypes(params);
        
        const regulationTypeData = response.data?.data || response.data?.[0]?.data || [];
        const meta = response.data?.[0]?.meta?.pagination || {};

        this.regulationTypes = regulationTypeData;
        this.totalItems = meta.total || regulationTypeData.length;
        
      } catch (error) {
        console.error("Error fetching jenis regulasi:", error);
        this.$swal({
            icon: 'error',
            title: 'Oops...',
            text: 'Gagal memuat data jenis regulasi. Silakan coba lagi nanti.',
        });
      }
    },
    advancedDeleteAlert(item) {
      this.$swal({
        title: `Hapus Jenis Regulasi "${item.namajenisregulasi}"`,
        ...
        if (result.isConfirmed) {
          try {
            await deleteRegulationType(item.idjenisregulasi);
            if (this.regulationTypes.length === 1 && this.currentPage > 1) {
              this.currentPage--;
            } else {
              this.fetchRegulationTypes();
            }
            this.toast.success("Data jenis regulasi berhasil dihapus", { icon: 'fa fa-check' });
          } catch (error) {
            this.toast.error("Gagal menghapus data jenis regulasi", { icon: 'fa fa-times' });
          }
        }
      });
    },
    ...
    resetFilters() {
      this.filters.namajenisregulasi = '';
      
      this.sortColumn = 'namajenisregulasi';
      this.sortDirection = 'asc';
      this.currentPage = 1;
      this.fetchRegulationTypes();
    },
