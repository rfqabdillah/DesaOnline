<template>
  <div class="col-sm-12">
    <add-edit-budgetSource-modal
      v-if="isModalVisible"
      :budgetSource-to-edit="budgetSourceBeingEdited"
      @close="closeModal"
    ...
    <div class="card">
      <div class="card-header">
        <h3>Daftar Nama Sumber Anggaran</h3>
      </div>
      ...
      <div class="card-body">
        ...
        <div v-if="isFilterVisible" class="border p-3 mb-3 rounded filter-section">
          <div class="row g-2">
            <div class="col-md-4">
              <label for="filterName" class="form-label">Nama Sumber Anggaran</label>
              <input type="text" id="filterName" class="form-control" v-model="filters.namasumberanggaran" placeholder="Filter berdasarkan nama sumber anggaran">
            </div>
          </div>
      ...
      <div class="table-responsive signal-table">
        <table class="table table-hover">
          <thead>
            <tr>
              <th scope="col">No</th>
              <th scope="col" @click="sortBy('namasumberanggaran')">
                Nama Sumber Anggaran
                <i class="fa fa-sort-asc" v-if="sortColumn === 'namasumberanggaran' && sortDirection === 'asc'"></i>
                <i class="fa fa-sort-desc" v-else-if="sortColumn === 'namasumberanggaran' && sortDirection === 'desc'"></i>
          ...
          <tbody>
            <tr v-for="(item, index) in paginatedBudgetSources" :key="item.idsumberanggaran">
              <th scope="row">{{ (currentPage - 1) * perPage + index + 1 }}</th>
              <td>{{ item.namasumberanggaran }}</td>
            ...
            <tr v-if="paginatedBudgetSources.length === 0">
...
<script>
import { getBudgetSources, deleteBudgetSource } from '@/services/referensi/budgetSources';
import AddEditBudgetSourceModal from './addEditBudgetSourceModal.vue';
import { useToast } from "vue-toastification";

export default {
  components: { AddEditBudgetSourceModal },
  data() {
    return {
      budgetSources: [],
      sortColumn: 'namasumberanggaran',
      sortDirection: 'asc',
      isModalVisible: false,
      currentPage: 1,
      perPage: 20,
      totalItems: 0,
      budgetSourceBeingEdited: null,
      jumpPage: 1,
  ...
  computed: {
    ...
    paginatedBudgetSources() {
      return this.budgetSources;
    },
    totalPages() {
  ...
  methods: {
    ...
    openAddModal() {
      this.budgetSourceBeingEdited = null;
      this.isModalVisible = true;
    },
    openEditModal(item) {
      this.budgetSourceBeingEdited = { ...item };
      this.isModalVisible = true;
    },
    closeModal() {
    ...
    async fetchBudgetSources() {
      try {
        ...
        const response = await getBudgetSources(params);
        
        const budgetSourceData = response.data?.data || response.data?.[0]?.data || [];
        const meta = response.data?.[0]?.meta?.pagination || {};

        this.budgetSources = budgetSourceData;
        this.totalItems = meta.total || budgetSourceData.length;
        ...
        this.$swal({
            icon: 'error',
            title: 'Oops...',
            text: 'Gagal memuat data sumber anggaran. Silakan coba lagi nanti.',
        });
      }
    },
    advancedDeleteAlert(item) {
      this.$swal({
        title: `Hapus Sumber Anggaran "${item.namasumberanggaran}"`,
        ...
        if (result.isConfirmed) {
          try {
            await deleteBudgetSource(item.idsumberanggaran);
            if (this.budgetSources.length === 1 && this.currentPage > 1) {
              this.currentPage--;
            } else {
              this.fetchBudgetSources();
            }
            this.toast.success("Data sumber anggaran berhasil dihapus", { icon: 'fa fa-check' });
          } catch (error) {
            this.toast.error("Gagal menghapus data sumber anggaran", { icon: 'fa fa-times' });
          }
        }
      });
    },
    ...
    resetFilters() {
      this.filters.namasumberanggaran = '';
      
      this.sortColumn = 'namasumberanggaran';
      this.sortDirection = 'asc';
      this.currentPage = 1;
      this.fetchBudgetSources();
    },
