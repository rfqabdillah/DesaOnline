<template>
  <div class="col-sm-12">
    <add-edit-groupCategory-modal
      v-if="isModalVisible"
      :groupCategory-to-edit="groupCategoryBeingEdited"
      @close="closeModal"
    ...
    <div class="card">
      <div class="card-header">
        <h3>Daftar Kategori Kelompok</h3>
      </div>
      ...
      <div class="card-body">
        ...
        <div v-if="isFilterVisible" class="border p-3 mb-3 rounded filter-section">
          <div class="row g-2">
            <div class="col-md-4">
              <label for="filterName" class="form-label">Kategori Kelompok</label>
              <input type="text" id="filterName" class="form-control" v-model="filters.namakategorikelompok" placeholder="Filter berdasarkan nama kategori kelompok">
            </div>
          </div>
      ...
      <div class="table-responsive signal-table">
        <table class="table table-hover">
          <thead>
            <tr>
              <th scope="col">No</th>
              <th scope="col" @click="sortBy('namakategorikelompok')">
                Kategori Kelompok
                <i class="fa fa-sort-asc" v-if="sortColumn === 'namakategorikelompok' && sortDirection === 'asc'"></i>
                <i class="fa fa-sort-desc" v-else-if="sortColumn === 'namakategorikelompok' && sortDirection === 'desc'"></i>
          ...
          <tbody>
            <tr v-for="(item, index) in paginatedGroupCategories" :key="item.idkategorikelompok">
              <th scope="row">{{ (currentPage - 1) * perPage + index + 1 }}</th>
              <td>{{ item.namakategorikelompok }}</td>
            ...
            <tr v-if="paginatedGroupCategories.length === 0">
...
<script>
import { getGroupCategories, deleteGroupCategory } from '@/services/referensi/groupCategories';
import AddEditGroupCategoryModal from './addEditGroupCategoryModal.vue';
import { useToast } from "vue-toastification";

export default {
  components: { AddEditGroupCategoryModal },
  data() {
    return {
      groupCategories: [],
      sortColumn: 'namakategorikelompok',
      sortDirection: 'asc',
      isModalVisible: false,
      currentPage: 1,
      perPage: 20,
      totalItems: 0,
      groupCategoryBeingEdited: null,
      jumpPage: 1,
  ...
  methods: {
    ...
    openAddModal() {
      this.groupCategoryBeingEdited = null;
      this.isModalVisible = true;
    },
    openEditModal(item) {
      this.groupCategoryBeingEdited = { ...item };
      this.isModalVisible = true;
    },
    closeModal() {
    ...
    async fetchGroupCategories() {
      try {
        ...
        const response = await getGroupCategories(params);
        
        const groupCategoryData = response.data?.data || response.data?.[0]?.data || [];
        const meta = response.data?.[0]?.meta?.pagination || {};

        this.groupCategories = groupCategoryData;
        this.totalItems = meta.total || groupCategoryData.length;
        
      } catch (error) {
        console.error("Error fetching kategori kelompok:", error);
        this.$swal({
            icon: 'error',
            title: 'Oops...',
            text: 'Gagal memuat data kategori kelompok. Silakan coba lagi nanti.',
        });
      }
    },
    advancedDeleteAlert(item) {
      this.$swal({
        title: `Hapus Kategori "${item.namakategorikelompok}"`,
        ...
        if (result.isConfirmed) {
          try {
            await deleteGroupCategory(item.idkategorikelompok);
            if (this.groupCategories.length === 1 && this.currentPage > 1) {
              this.currentPage--;
            } else {
              this.fetchGroupCategories();
            }
            this.toast.success("Data kategori kelompok berhasil dihapus", { icon: 'fa fa-check' });
          } catch (error) {
            this.toast.error("Gagal menghapus data kategori kelompok", { icon: 'fa fa-times' });
          }
        }
      });
    },
    ...
    resetFilters() {
      this.filters.namakategorikelompok = '';
      
      this.sortColumn = 'namakategorikelompok';
      this.sortDirection = 'asc';
      this.currentPage = 1;
      this.fetchGroupCategories();
    },
