<template>
  <div class="col-sm-12">
    <add-edit-gender-modal
      v-if="isModalVisible"
      :gender-to-edit="genderBeingEdited"
      @close="closeModal"
    ...
    <div class="card">
      <div class="card-header">
        <h3>Daftar Nama Jenis Kelamin</h3>
      </div>
      ...
      <div class="card-body">
        ...
        <div v-if="isFilterVisible" class="border p-3 mb-3 rounded filter-section">
          <div class="row g-2">
            <div class="col-md-4">
              <label for="filterName" class="form-label">Nama Jenis Kelamin</label>
              <input type="text" id="filterName" class="form-control" v-model="filters.namajeniskelamin" placeholder="Filter berdasarkan nama jenis kelamin">
            </div>
          </div>
      ...
      <div class="table-responsive signal-table">
        <table class="table table-hover">
          ...
          <tbody>
            <tr v-for="(item, index) in paginatedGenders" :key="item.idjeniskelamin">
              <th scope="row">{{ (currentPage - 1) * perPage + index + 1 }}</th>
              <td>{{ item.namajeniskelamin }}</td>
              <td>
                <div class="btn-group">
                  <button class="btn btn-primary btn-sm" @click="openEditModal(item)">
                    <i class="fa fa-pencil"></i>
                  </button>
                  <button class="btn btn-danger sweet-11 btn-sm" type="button" @click="advancedDeleteAlert(item.idjeniskelamin)">
                    <i class="fa fa-trash"></i>
                  </button>
                </div>
              </td>
            </tr>
            <tr v-if="paginatedGenders.length === 0">
...
<script>
import { getGenders, deleteGender } from '@/services/referensi/genders';
import AddEditGenderModal from './addEditGenderModal.vue';
import { useToast } from "vue-toastification";

export default {
  components: { AddEditGenderModal },
  data() {
    return {
      genders: [],
      sortColumn: '',
  ...
  methods: {
    openAddModal() {
      this.genderBeingEdited = null;
      this.isModalVisible = true;
    },
    openEditModal(item) {
      this.genderBeingEdited = { ...item };
      this.isModalVisible = true;
    },
    closeModal() {
    ...
    async fetchGenders() {
      try {
        ...
        const genderData = response.data?.data || response.data?.[0]?.data || [];
        const meta = response.data?.[0]?.meta?.pagination || {};

        this.genders = genderData;
        this.totalItems = meta.total || genderData.length;
        ...
        this.$swal({
            icon: 'error',
            title: 'Oops...',
            text: 'Gagal memuat data jenis kelamin. Silakan coba lagi nanti.',
        });
      }
    },
    advancedDeleteAlert(id) {
      this.$swal({
        title: 'Hapus Data Jenis Kelamin',
        ...
        if (result.isConfirmed) {
          try {
            ...
            if (this.genders.length === 1 && this.currentPage > 1) {
              this.currentPage--;
            } else {
              this.fetchGenders();
            }
            this.toast.success("Data jenis kelamin berhasil dihapus", { icon: 'fa fa-check' });
          } catch (error) {
            this.toast.error("Gagal menghapus data jenis kelamin", { icon: 'fa fa-times' });
          }
        }
      });
    },
