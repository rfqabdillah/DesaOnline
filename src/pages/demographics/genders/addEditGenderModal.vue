  ...
  <div class="modal-overlay" @click="handleOverlayClick">
    <div class="modal-content" @click.stop>
      <div class="modal-header">
        <h5 class="modal-title">{{ isEditMode ? 'Edit Data Jenis Kelamin' : 'Tambah Data Jenis Kelamin' }}</h5>
      ...
      <div class="modal-body">
        <form @submit.prevent="submitForm">
          <div class="mb-3">
            <label for="namajeniskelamin" class="form-label">Nama Jenis Kelamin</label>
            <input 
              type="text" 
              class="form-control" 
              id="namaJenisKelamin" 
              placeholder="Masukkan nama jenis kelamin"
              v-model="formData.namajeniskelamin" required>
          </div>
...
export default {
  ...
  methods: {
    ...
    async submitForm() {
      ...
      const { namajeniskelamin } = this.formData;
      if ( !namajeniskelamin ) {
      ...
      try {
        if (this.isEditMode) {
          // DIUBAH: Menggunakan this.formData
          await updateGender(this.genderToEdit.idjeniskelamin, this.formData);
          this.toast.success("Data jenis kelamin berhasil diperbarui", { icon: 'fa fa-check' });
        } else {
          // DIUBAH: Menggunakan this.formData
          await addGender(this.formData);
          this.toast.success("Data jenis kelamin berhasil ditambah", { icon: 'fa fa-check' });
        }
        ...
        this.toast.error("Gagal menyimpan data jenis kelamin", { icon: 'fa fa-times' });
      } finally {
