<template>
  <div class="col-sm-12">
    <add-edit-educationLevel-modal
      v-if="isModalVisible"
      :educationLevel-to-edit="educationLevelBeingEdited"
      @close="closeModal"
    ...
    <div class="card">
      ...
      <div class="card-body">
        ...
        <div v-if="isFilterVisible" class="border p-3 mb-3 rounded filter-section">
          <div class="row g-2">
            <div class="col-md-4">
              <label for="filterName" class="form-label">Nama Pendidikan</label>
              <input type="text" id="filterName" class="form-control" v-model="filters.namapendidikan" placeholder="Filter berdasarkan nama pendidikan">
            </div>
          </div>
      ...
      <div class="table-responsive signal-table">
        <table class="table table-hover">
          <thead>
            <tr>
              <th scope="col">No</th>
              <th scope="col" @click="sortBy('namapendidikan')">
                Nama Pendidikan
                <i class="fa fa-sort-asc" v-if="sortColumn === 'namapendidikan' && sortDirection === 'asc'"></i>
                <i class="fa fa-sort-desc" v-else-if="sortColumn === 'namapendidikan' && sortDirection === 'desc'"></i>
          ...
          <tbody>
            <tr v-for="(item, index) in paginatedEducationLevels" :key="item.idpendidikan">
            ...
            <tr v-if="paginatedEducationLevels.length === 0">
...
<script>
import { getEducationLevels, deleteEducationLevel } from '@/services/referensi/educationLevels';
import AddEditEducationLevelModal from './addEditEducationLevelModal.vue';
import { useToast } from "vue-toastification";

export default {
  components: { AddEditEducationLevelModal },
  data() {
    return {
      educationLevels: [],
      sortColumn: 'namapendidikan',
      sortDirection: 'asc',
      isModalVisible: false,
      currentPage: 1,
      perPage: 20,
      totalItems: 0,
      educationLevelBeingEdited: null,
      jumpPage: 1,
  ...
  methods: {
    ...
    openAddModal() {
      this.educationLevelBeingEdited = null;
      this.isModalVisible = true;
    },
    openEditModal(item) {
      this.educationLevelBeingEdited = { ...item };
      this.isModalVisible = true;
    },
    closeModal() {
    ...
    async fetchEducationLevels() {
      try {
        ...
        const educationLevelData = response.data?.data || response.data?.[0]?.data || [];
        const meta = response.data?.[0]?.meta?.pagination || {};

        this.educationLevels = educationLevelData;
        this.totalItems = meta.total || educationLevelData.length;
        ...
        this.$swal({
            icon: 'error',
            title: 'Oops...',
            text: 'Gagal memuat data pendidikan. Silakan coba lagi nanti.',
        });
      }
    },
    advancedDeleteAlert(item) {
      this.$swal({
        title: `Hapus Pendidikan "${item.namapendidikan}"`,
        ...
        if (result.isConfirmed) {
          try {
            await deleteEducationLevel(item.idpendidikan);
            if (this.educationLevels.length === 1 && this.currentPage > 1) {
              this.currentPage--;
            } else {
              this.fetchEducationLevels();
            }
            this.toast.success("Data pendidikan berhasil dihapus", { icon: 'fa fa-check' });
          } catch (error) {
            this.toast.error("Gagal menghapus data pendidikan", { icon: 'fa fa-times' });
          }
        }
      });
    },
