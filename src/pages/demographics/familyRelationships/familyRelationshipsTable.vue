<template>
  <div class="col-sm-12">
    <add-edit-familyRelationship-modal
      v-if="isModalVisible"
      :familyRelationship-to-edit="familyRelationshipBeingEdited"
      @close="closeModal"
    ...
    <div class="card">
      <div class="card-header">
        <h3>Daftar Nama Hubungan Keluarga</h3>
      </div>
      ...
      <div class="card-body">
        ...
        <div v-if="isFilterVisible" class="border p-3 mb-3 rounded filter-section">
          <div class="row g-2">
            <div class="col-md-4">
              <label for="filterName" class="form-label">Nama Hubungan Keluarga</label>
              <input type="text" id="filterName" class="form-control" v-model="filters.namahubungankeluarga" placeholder="Filter berdasarkan nama hubungan keluarga">
            </div>
          </div>
      ...
      <div class="table-responsive signal-table">
        <table class="table table-hover">
          <thead>
            <tr>
              <th scope="col">No</th>
              <th scope="col" @click="sortBy('namahubungankeluarga')">
                Nama Hubungan Keluarga
                <i class="fa fa-sort-asc" v-if="sortColumn === 'namahubungankeluarga' && sortDirection === 'asc'"></i>
...
<script>
import { getFamilyRelationships, deleteFamilyRelationship } from '@/services/referensi/familyRelationships';
import AddEditFamilyRelationshipModal from './addEditFamilyRelationshipModal.vue';
import { useToast } from "vue-toastification";

export default {
  components: { AddEditFamilyRelationshipModal },
  data() {
    return {
      familyRelationships: [],
      sortColumn: '',
      ...
      perPage: 20,
      totalItems: 0,
      familyRelationshipBeingEdited: null,
      jumpPage: 1,
  ...
  methods: {
    ...
    openEditModal(item) {
      this.familyRelationshipBeingEdited = { ...item };
      this.isModalVisible = true;
    },
    closeModal() {
    ...
    async fetchFamilyRelationships() {
      try {
        const params = {
        ...
        const response = await getFamilyRelationships(params);
        
        const familyRelationshipData = response.data?.data || response.data?.[0]?.data || [];
        const meta = response.data?.[0]?.meta?.pagination || {};

        this.familyRelationships = familyRelationshipData;
        this.totalItems = meta.total || familyRelationshipData.length;
        
      } catch (error) {
        console.error("Error fetching hubungan keluarga:", error);
        this.$swal({
            icon: 'error',
            title: 'Oops...',
            text: 'Gagal memuat data hubungan keluarga. Silakan coba lagi nanti.',
        });
      }
    },
      ...
      this.$swal({
        ...
        if (result.isConfirmed) {
          try {
            ...
            this.toast.success("Data hubungan keluarga berhasil dihapus", { icon: 'fa fa-check' });
          } catch (error) {
            this.toast.error("Gagal menghapus data hubungan keluarga", { icon: 'fa fa-times' });
          }
        }
      });
    },
