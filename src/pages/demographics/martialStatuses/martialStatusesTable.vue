...
export default {
  ...
  data() {
    return {
      martialStatuses: [],
      sortColumn: 'namastatusperkawinan',
      sortDirection: 'asc',
  ...
  methods: {
    ...
    advancedDeleteAlert(item) {
      this.$swal({
        title: `Hapus status "${item.namastatusperkawinan}"`,
        ...
        if (result.isConfirmed) {
          try {
            await deleteMartialStatus(item.idstatusperkawinan);
    ...
    resetFilters() {
      ...
      this.sortColumn = 'namastatusperkawinan';
      this.sortDirection = 'asc';
      this.currentPage = 1;
      this.fetchMartialStatuses();
    },
