<template>
  <div class="col-sm-12">
    <add-edit-birthPlace-modal
      v-if="isModalVisible"
      :birthPlace-to-edit="birthPlaceBeingEdited"
      @close="closeModal"
    ...
    <div class="card">
      <div class="card-header">
        <h3>Daftar Tempat Dilahirkan</h3>
      </div>
      ...
      <div class="card-body">
        ...
        <div v-if="isFilterVisible" class="border p-3 mb-3 rounded filter-section">
          <div class="row g-2">
            <div class="col-md-4">
              <label for="filterName" class="form-label">Tempat Dilahirkan</label>
              <input type="text" id="filterName" class="form-control" v-model="filters.namatempatdilahirkan" placeholder="Filter berdasarkan tempat dilahirkan">
            </div>
          </div>
      ...
      <div class="table-responsive signal-table">
        <table class="table table-hover">
          <thead>
            <tr>
              <th scope="col">No</th>
              <th scope="col" @click="sortBy('namatempatdilahirkan')">
                Tempat Dilahirkan
                <i class="fa fa-sort-asc" v-if="sortColumn === 'namatempatdilahirkan' && sortDirection === 'asc'"></i>
...
<script>
import { getBirthPlaces, deleteBirthPlace } from '@/services/referensi/birthPlaces';
import AddEditBirthPlaceModal from './addEditBirthPlaceModal.vue';
import { useToast } from "vue-toastification";

export default {
  components: { AddEditBirthPlaceModal },
  data() {
    return {
      birthPlaces: [],
      sortColumn: '',
  ...
  methods: {
    ...
    openEditModal(item) {
      this.birthPlaceBeingEdited = { ...item };
      this.isModalVisible = true;
    },
    closeModal() {
    ...
    async fetchBirthPlaces() {
      try {
        ...
        const birthPlaceData = response.data?.data || response.data?.[0]?.data || [];
        const meta = response.data?.[0]?.meta?.pagination || {};

        this.birthPlaces = birthPlaceData;
        ...
        this.$swal({
            icon: 'error',
            title: 'Oops...',
            text: 'Gagal memuat data tempat dilahirkan. Silakan coba lagi nanti.',
        });
      }
    },
      ...
      this.$swal({
        ...
        if (result.isConfirmed) {
          try {
            ...
            this.toast.success("Data tempat dilahirkan berhasil dihapus", { icon: 'fa fa-check' });
          } catch (error) {
            this.toast.error("Gagal menghapus data tempat dilahirkan", { icon: 'fa fa-times' });
          }
        }
      });
    },
    ...
    resetFilters() {
      this.filters.namatempatdilahirkan = '';
