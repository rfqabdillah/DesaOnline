<template>
  <div class="col-sm-12">
    <add-edit-bloodType-modal
      v-if="isModalVisible"
      :bloodType-to-edit="bloodTypeBeingEdited"
      @close="closeModal"
    ...
    <div class="card">
      ...
      <div class="card-body">
        ...
        <div v-if="isFilterVisible" class="border p-3 mb-3 rounded filter-section">
          <div class="row g-2">
            <div class="col-md-4">
              <label for="filterName" class="form-label">Golongan Darah</label>
              <input type="text" id="filterName" class="form-control" v-model="filters.namagolongandarah" placeholder="Filter berdasarkan golongan darah">
            </div>
          </div>
      ...
      <div class="table-responsive signal-table">
        <table class="table table-hover">
          <thead>
            <tr>
              <th scope="col">No</th>
              <th scope="col" @click="sortBy('namagolongandarah')">
                Golongan Darah
                <i class="fa fa-sort-asc" v-if="sortColumn === 'namagolongandarah' && sortDirection === 'asc'"></i>
                <i class="fa fa-sort-desc" v-else-if="sortColumn === 'namagolongandarah' && sortDirection === 'desc'"></i>
          ...
          <tbody>
            <tr v-for="(item, index) in paginatedBloodTypes" :key="item.idgolongandarah">
              <th scope="row">{{ (currentPage - 1) * perPage + index + 1 }}</th>
              <td>{{ item.namagolongandarah }}</td>
            ...
            <tr v-if="paginatedBloodTypes.length === 0">
...
<script>
import { getBloodTypes, deleteBloodType } from '@/services/referensi/bloodTypes';
import AddEditBloodTypeModal from './addEditBloodTypeModal.vue';
import { useToast } from "vue-toastification";

export default {
  components: { AddEditBloodTypeModal },
  data() {
    return {
      bloodTypes: [],
      sortColumn: 'namagolongandarah',
      sortDirection: 'asc',
      isModalVisible: false,
      currentPage: 1,
      perPage: 20,
      totalItems: 0,
      bloodTypeBeingEdited: null,
      jumpPage: 1,
  ...
  methods: {
    ...
    openAddModal() {
      this.bloodTypeBeingEdited = null;
      this.isModalVisible = true;
    },
    openEditModal(item) {
      this.bloodTypeBeingEdited = { ...item };
      this.isModalVisible = true;
    },
    closeModal() {
    ...
    async fetchBloodTypes() {
      try {
        ...
        const response = await getBloodTypes(params);
        
        const bloodTypeData = response.data?.data || response.data?.[0]?.data || [];
        const meta = response.data?.[0]?.meta?.pagination || {};

        this.bloodTypes = bloodTypeData;
        this.totalItems = meta.total || bloodTypeData.length;
        
      } catch (error) {
        console.error("Error fetching golongan darah:", error);
        this.$swal({
            icon: 'error',
            title: 'Oops...',
            text: 'Gagal memuat data golongan darah. Silakan coba lagi nanti.',
        });
      }
    },
    advancedDeleteAlert(item) {
      this.$swal({
        title: `Hapus Golongan Darah "${item.namagolongandarah}"`,
        ...
        if (result.isConfirmed) {
          try {
            await deleteBloodType(item.idgolongandarah);
            if (this.bloodTypes.length === 1 && this.currentPage > 1) {
              this.currentPage--;
            } else {
              this.fetchBloodTypes();
            }
            this.toast.success("Data golongan darah berhasil dihapus", { icon: 'fa fa-check' });
          } catch (error) {
            this.toast.error("Gagal menghapus data golongan darah", { icon: 'fa fa-times' });
          }
        }
      });
    },
    ...
    resetFilters() {
      this.filters.namagolongandarah = '';
      
      this.sortColumn = 'namagolongandarah';
      this.sortDirection = 'asc';
      this.currentPage = 1;
      this.fetchBloodTypes();
    },
