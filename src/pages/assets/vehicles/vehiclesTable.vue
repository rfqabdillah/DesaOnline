<template>
  <div class="col-sm-12">
    ...
    <detail-vehicle-modal
      ...
      :vehicle-to-view="vehicleBeingViewed"
        ...
        <table class="table table-hover">
          <thead>
            <tr>
              ...
              <th scope="col" @click="sortBy('nomorpolisi')">
                Nomor Polisi
                <i class="fa fa-sort-asc" v-if="sortColumn === 'nomorpolisi' && sortDirection === 'asc'"></i>
                <i class="fa fa-sort-desc" v-else-if="sortColumn === 'nomorpolisi' && sortDirection === 'desc'"></i>
                <i class="fa fa-sort" v-else></i>
              </th>
          ...
          <tbody>
            ...
            <template v-else>
              <tr v-for="(item, index) in paginatedVehicles" :key="item.idkendaraan">
...
<script>
import { deleteVehicle, getVehicles } from '@/services/general/assets/vehicles';
import { getConditions } from '@/services/referensi/conditions'; 
import AddEditVehicleModal from './addEditVehicleModal.vue';
import DetailVehicleModal from './detailVehicle.vue';
import { useToast } from "vue-toastification";

export default {
  components: { AddEditVehicleModal, DetailVehicleModal }, 
  data() {
    return {
      vehicles: [],
      kondisiList: [], 
  ...
  methods: {
    ...
    openDetailModal(item) {
      this.vehicleBeingViewed = { ...item };
      this.isDetailModalVisible = true;
    },
    closeDetailModal() {
      this.isDetailModalVisible = false;
      this.vehicleBeingViewed = null;
    },
    ...
    async fetchVehicles() {
      ...
      try {
        ...
        const vehicleData = response.data?.data || response.data?.[0]?.data || [];
        const meta = response.data?.meta?.pagination || {};

        this.vehicles = vehicleData;
        this.totalItems = meta.total || vehicleData.length;
        
      } catch (error) {
        console.error("Error fetching vehicles:", error);
        this.toast.error("Gagal memuat data kendaraan", { icon: 'fa fa-times' });
      } finally {
      ...
      this.$swal({
        ...
        if (result.isConfirmed) {
          try {
            ...
            this.toast.success("Data kendaraan berhasil dihapus", { icon: 'fa fa-check' });
          } catch (error) {
            this.toast.error("Gagal menghapus data kendaraan", { icon: 'fa fa-times' });
          }
        }
      });
    },
