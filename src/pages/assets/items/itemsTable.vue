    ...
    <div class="card">
      ...
      <div class="card-body">
        ...
        <div v-if="isFilterVisible" class="border p-3 mb-3 rounded filter-section">
          <div class="row g-3">
            <div class="col-md-3">
              <label for="filterItemName" class="form-label">Nama Barang</label>
              <input type="text" id="filterItemName" class="form-control" v-model="filters.namabarang" placeholder="Filter berdasarkan nama barang">
            </div>
            <div class="col-md-3">
              <label for="filterPoliceNumber" class="form-label">Kode Barang</label>
              <input type="text" id="filterPoliceNumber" class="form-control" v-model="filters.kodebarang" placeholder="Filter berdasarkan kode barang">
            </div>
            <div class="col-md-3">
            ...
            <div class="col-md-3">
              ...
              <select id="filterCondition" class="form-select" v-model="filters.idkondisi">
                <option value="">Semua Kondisi</option>
                <option v-for="kondisi in kondisiList" :key="kondisi.idkondisi" :value="kondisi.idkondisi">
                  {{ kondisi.namakondisi }}
                </option>
              </select>
            </div>
      ...
      <div class="table-responsive signal-table">
        <table class="table table-hover">
          <thead>
            <tr>
              <th scope="col">No</th>
              <th scope="col" @click="sortBy('namabarang')">
                Nama Barang / Merk
                <i class="fa fa-sort-asc" v-if="sortColumn === 'namabarang' && sortDirection === 'asc'"></i>
                <i class="fa fa-sort-desc" v-else-if="sortColumn === 'namabarang' && sortDirection === 'desc'"></i>
                <i class="fa fa-sort" v-else></i>
              </th>
              <th scope="col" @click="sortBy('kodebarang')">
                Kode Barang
                <i class="fa fa-sort-asc" v-if="sortColumn === 'kodebarang' && sortDirection === 'asc'"></i>
                <i class="fa fa-sort-desc" v-else-if="sortColumn === 'kodebarang' && sortDirection === 'desc'"></i>
                <i class="fa fa-sort" v-else></i>
              </th>
          ...
          <tbody>
            ...
            <template v-else>
              <tr v-for="(item, index) in paginatedItems" :key="item.idbarang">
                <th scope="row">{{ (currentPage - 1) * perPage + index + 1 }}</th>
                <td>{{ item.namabarang }} <br> <small class="text-muted">{{ item.merk }}</small></td>
                ...
                <td>
                  <div class="btn-group">
                    ...
                    <button class="btn btn-danger sweet-11 btn-sm" type="button" @click="advancedDeleteAlert(item.idbarang)" title="Hapus Data">
...
<script>
import { deleteItem, getItems } from '@/services/general/assets/items';
import { getConditions } from '@/services/referensi/conditions'; 
import AddEditItemModal from './addEditItemModal.vue';
import DetailItemModal from './detailItem.vue';
import { useToast } from "vue-toastification";

export default {
  components: { AddEditItemModal, DetailItemModal }, 
  data() {
    return {
      items: [],
      kondisiList: [], 
  ...
  methods: {
    ...
    async fetchItems() {
      ...
      try {
        ...
        const itemData = response.data?.data || response.data?.[0]?.data || [];
        const meta = response.data?.meta?.pagination || {};

        this.items = itemData;
        this.totalItems = meta.total || itemData.length;
        
      } catch (error) {
        console.error("Error fetching items:", error);
        this.toast.error("Gagal memuat data barang", { icon: 'fa fa-times' });
      } finally {
    ...
    async fetchConditions() {
      try {
        ...
        this.kondisiList = response.data?.[0]?.data || [];
      } catch (error) {
      ...
      this.$swal({
        ...
        if (result.isConfirmed) {
          try {
            ...
            this.toast.success("Data barang berhasil dihapus", { icon: 'fa fa-check' });
          } catch (error) {
            this.toast.error("Gagal menghapus data barang", { icon: 'fa fa-times' });
          }
        }
      });
    },
    ...
    resetFilters() {
      this.filters.namabarang = '';
      this.filters.kodebarang = '';
      this.filters.namawilayah = '';
      this.filters.idkondisi = '';
