<template>
  <div class="col-sm-12">
    <add-edit-region-modal
      v-if="isModalVisible"
      :region-to-edit="regionBeingEdited"
      @close="closeModal"
        ...
        <div v-if="isFilterVisible" class="border p-3 mb-3 rounded filter-section">
          <div class="row g-3">
            <div class="col-md-4">
              ...
              <input type="text" id="filterCode" class="form-control" v-model="filters.region_code" placeholder="Filter berdasarkan kode wilayah">
            </div>
            <div class="col-md-4">
              <label for="filterName" class="form-label">Nama Wilayah</label>
              <input type="text" id="filterName" class="form-control" v-model="filters.region_name" placeholder="Filter berdasarkan nama wilayah">
            </div>
            <div class="col-md-4">
              <label for="filterType" class="form-label">Tipe Wilayah</label>
              <select id="filterType" class="form-select" v-model="filters.region_type">
        ...
        <table class="table table-hover">
          <thead>
            <tr>
              ...
              <th scope="col" @click="sortBy('region_type')">
                Tipe Wilayah
                <i class="fa fa-sort-asc" v-if="sortColumn === 'region_type' && sortDirection === 'asc'"></i>
          ...
          <tbody>
            <tr v-for="(item, index) in paginatedRegions" :key="item.region_id">
...
import AddEditRegionModal from './addEditRegionModal.vue';
import { useToast } from "vue-toastification";

export default {
  components: { AddEditRegionModal },
  data() {
    return {
      regions: [],
      sortColumn: '',
    ...
    async fetchRegions() {
      try {
        ...
        this.$swal({
            icon: 'error',
            title: 'Oops...',
            text: 'Gagal memuat data wilayah. Silakan coba lagi nanti.',
        });
      }
    },
      ...
      this.$swal({
        ...
        if (result.isConfirmed) {
          try {
            ...
            this.toast.success("Data wilayah berhasil dihapus", { icon: 'fa fa-check' });
          } catch (error) {
            this.toast.error("Gagal menghapus data wilayah", { icon: 'fa fa-times' });
          }
        }
      });
    },
