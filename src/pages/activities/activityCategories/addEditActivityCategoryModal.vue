    ...
    <div class="modal-overlay" @click="handleOverlayClick">
      <div class="modal-content" @click.stop>
        <div class="modal-header">
          <h5 class="modal-title">{{ isEditMode ? 'Edit Data Kategori Kegiatan' : 'Tambah Data Kategori Kegiatan' }}</h5>
        ...
        <div class="modal-body">
          <form @submit.prevent="submitForm">
            <div class="col mb-3">
              ...
              <input type="text" class="form-control" v-model="formData.namakategorikegiatan" placeholder="Masukkan nama kategori kegiatan" required />
            </div>
            <div class="mb-3">
...
<script>
import { addCategory, updateCategory } from '@/services/general/activities/activityCategories'; 
...
export default {
  name: 'addEditActivityCategoryModal',
  props: {
    activityCategoryToEdit: { type: Object, default: null },
  },
  data() {
    return {
  ...
  computed: {
    isEditMode() {
      return !!this.activityCategoryToEdit;
    }
  },
  watch: {
    activityCategoryToEdit: {
      handler(newData) {
        if (newData) {
          this.formData.iddesa = newData.id_desa || newData.iddesa || '';
          this.formData.namakategorikegiatan = newData.namakategorikegiatan;
        } else {
  ...
  methods: {
    ...
    async submitForm() {
      ...
      data.append('record[namakategorikegiatan]', this.formData.namakategorikegiatan);
      ...
      try {
        ...
        if (this.isEditMode) {
            ...
            response = await updateCategory(this.activityCategoryToEdit.idkategoriartikel, data);
            this.toast.success("Data kategori kegiatan berhasil diperbarui");
        } else {
            response = await addCategory(data);
            this.toast.success("Data kategori kegiatan berhasil ditambah");
        }
        ...
        this.toast.error("Gagal menyimpan data kategori kegiatan");
      } finally {
