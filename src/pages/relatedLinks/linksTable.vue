      ...
      <div class="table-responsive signal-table">
        <table class="table table-hover">
          ...
          <tbody>
            ...
            <template v-else>
              <tr v-for="(item, index) in paginatedLinks" :key="item.idlink">
                <th scope="row">{{ (currentPage - 1) * perPage + index + 1 }}</th>
                <td>
                  <img :src="item.logolink" alt="Logo" style="height: 50px; width: auto;" v-if="item.logolink" />
...
<script>
import { getRelatedLinks, deleteRelatedLink } from '@/services/general/relatedLinks/relatedLinks'; 
import AddEditLinkModal from './addEditLinkModal.vue';
import { useToast } from "vue-toastification";

export default {
  components: { AddEditLinkModal }, 
  data() {
    return {
      links: [],
      isLoading: false, 
  ...
  methods: {
    ...
    async fetchLinks() {
      ...
      try {
        ...
        const response = await getRelatedLinks(params);
        const linkData = response.data?.data || response.data?.[0]?.data || [];
        const meta = response.data?.meta?.pagination || response.data?.[0]?.meta?.pagination || {};

        this.links = linkData;
        this.totalItems = meta.total || linkData.length;
        
      } catch (error) {
        console.error("Error fetching related links:", error);
        this.toast.error("Gagal memuat data link terkait", { icon: 'fa fa-times' });
      } finally {
      ...
      this.$swal({
        ...
        if (result.isConfirmed) {
          try {
            ...
            this.toast.success("Data link terkait berhasil dihapus", { icon: 'fa fa-check' });
          } catch (error) {
            this.toast.error("Gagal menghapus data link terkait", { icon: 'fa fa-times' });
          }
        }
      });
    },
